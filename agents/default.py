...
import models
from utils.metric import accuracy, accuracy_au, AverageMeter, Timer
...
class NormalNN(nn.Module):
    ...
    def __init__(self, agent_config):
        ...
        self.valid_out_dim = 0  # Default: 'ALL' means all output nodes are active
...
def accumulate_acc(output, target, task, meter):
    if 'All' in output.keys(): # Single-headed model
        meter.update(accuracy(output['FER'], target), len(target))
...
def accumulate_acc_au(output, target, task, meter):
    if 'All' in output.keys(): # Single-headed model
        ...
        acc = ((output["AU"] == target)).sum().item() / (N_val*C_val) * 100
