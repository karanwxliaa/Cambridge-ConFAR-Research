...
class NormalNN(nn.Module):
    ...
    '''
    def __init__(self, agent_config):                                                                                               #add argument for task 
        '''
    ...
    def create_model(self):
        ...
        # The output of the model will be a dict: {task_name1:output1, task_name2:output2 ...}                               #Change output here
...
#                    flag = 1
            ...
            if val_loader != None:
                axx, _ = self.validation(val_loader)
            if axx > 90:
                break
...
def accumulate_acc(output, target, task, meter):
    if 'All' in output.keys(): # Single-headed model
        N_val,C_val = target.shape
        acc = ((output["All"] == target)).sum().item() / (N_val*C_val) * 100
